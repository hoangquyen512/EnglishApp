...
**Golden rule of ARIA:** *No ARIA is better than bad ARIA.* Prefer native elements — ` `, ` `, ` `, ` `, ` `, ` ` — they bring focus, keyboard, and semantics for free. Reach for ARIA only when no native element exists. Follow the [ARIA Authoring Practices Guide (APG)](https://www.w3.org/WAI/ARIA/apg/) patterns rather than inventing roles.

### Buttons
- **Use ` `** for actions, ` ` for navigation. A clickable ` ` is an accessibility bug.
...
### Forms & inputs
- **Every input has a persistent visible ` `** linked via `for`/`id`. Placeholder is **not** a label (it vanishes on input, fails contrast, breaks autofill).
- Group related fields with ` ` + ` ` (e.g. radio groups, address blocks).
...
### Dialog / Modal
- Prefer the native ` ` element with `showModal()` — it provides the top layer, backdrop, and Esc-to-close.
...
- `role="dialog"` + `aria-modal="true"` + `aria-labelledby` (title) / `aria-describedby` (body) if not using native ` `.
...
### Tables (data)
- Use real ` ` with ` `, ` `, ` `. Don't fake tables with divs.
- Right-align numbers, left-align text; use tabular figures.
- Sortable headers: ` ` inside ` `, expose state with `aria-sort="ascending|descending|none"`.
...
### Navigation
- Wrap in ` `; mark current with `aria-current="page"`.
...
- **Whole-card-clickable trap:** don't wrap the entire card in ` ` if it contains other links/buttons (invalid nesting, SR confusion). Use the "stretched link" pattern — a single real ` ` on the title with a pseudo-element overlay (`::after { position:absolute; inset:0 }`); keep secondary buttons above it with `position:relative; z-index:1`.
- Keep cards in a set visually consistent (equal heights via grid, consistent padding/radius).

### Disclosure / Accordion / Tabs
- Disclosure/accordion: a ` ` toggling a region — or just native ` `/` `.
...
- **Forced-colors mode (Windows High Contrast):** the OS overrides your colors with a user palette. Use the `forced-colors: active` media query and `system-color` keywords; ensure icons drawn with `background-image` get a `forced-color-adjust` fallback or a real ` `/text so they don't vanish. Test it.
...
| **Placeholder-as-label** inputs | Add persistent ` ` (§6) |
| **Clickable ` `s**, fake buttons/tables | Use native ` `/` `/` ` (§6) |
